	  ...
	  $('#approval').click(function() {
		  if(confirm("결재를 승인하시겠습니까?")){
			  if(confirm("승인 후에는 다시 결재가 불가능합니다. 그래도 승인하시겠습니까?")){
				  location.href='approval?paper_num=${overtime.paper_num}&classification=초과근무신청서&condition=approval'
			  }			  
		  }
	  });  
	  ...
	  $('#refusal').click(function() {
			if(confirm("결재를 거절하시겠습니까?")){
			 if(confirm("거절 후에는 다시 결재가 불가능합니다. 그래도 거절하시겠습니까?")){
				 location.href='approval?paper_num=${overtime.paper_num}&classification=초과근무신청서&condition=refusal'	  
				 }			  
			 }
	  ...
	  $('#delete').click(function() {
			if(confirm("해당 문서를 삭제하겠습니까?")){
			 if(confirm("삭제 후 다시 복구가 불가능 합니다. 그래도 삭제하시겠습니까?")){
				 location.href='delete?paper_num=${overtime.paper_num}&classification=초과근무신청서'	  
				 }			  
			 }
...
.modal-header {
  ...
  <table class="table" id="maintable">
    ...
    <tbody>
      <tr>
        <th>분류</th>        
        <td class="main_td">초과근무신청서</td>
      </tr>
      <tr>
        <th>문서번호</th>
        <td><c:out value="${overtime.paper_num}"/>번</td>
      </tr>
      <tr>
        <th>기안자</th>
        <td><c:out value="${overtime.name}"/></td>
      </tr>
      <tr>
        <th>부서/직급</th>
        <td>
        <c:out value="${overtime.department}"/>/
        <c:out value="${overtime.position}"/>
        </td>
      </tr>
      <tr>
        <th>결재현황</th>
        <td class="condition"><c:out value="${overtime.condition}"/></td>
      </tr>
      <tr>
        <th>작성일</th>
        <td><c:out value="${overtime.write_date}"/></td>
      </tr>
      <tr>
        <th>근무일자</th>
        <td><c:out value="${overtime.overtime_date}"/></td>
      </tr>
      <tr>
        <th>근무시간</th>
        <td><c:out value="${overtime.over_time}"/></td>
      </tr>
      <tr>
        <th>작업내용</th>
        <td>
        <textarea class="form-control" readonly="readonly" rows="5"><c:out value="${overtime.overtime_content}"/></textarea>
        </td> 
      </tr>
       <tr>
        <th>사유</th>
        <td>
        <textarea class="form-control" readonly="readonly" rows="5"><c:out value="${overtime.overtime_reason}"/></textarea>
        </td> 
      </tr>
    </tbody>
  </table>
